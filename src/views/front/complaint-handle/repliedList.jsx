...
export default function ComplaintHandleRepliedPage(props) {
    ...
    const getChatData = (replyFormId) => {
        setIsLoading(true);
        dispatch(getFormDataReplyService({ formDataId: replyFormId, formType: "DataComplaint" }))
            .then((res) => {
    ...
    return (
        <MainContentPart className="h-[calc(100vh-88px)]">
                    ...
                    >
                        <CustomCard className="p-4 !rounded-none h-full">
                            <div className="card-header flex items-center justify-between gap-4 border-b border-light-850/20 pb-4 mb-4">
                                <h2 className="text-xl font-semibold text-primary">{formDataSelect?.mandatedPerson?.idNumber}</h2>
                            </div>
