                    ...
                    <tr key={index} className="bg-secondary/35 [&>td:last-child]:border-e-0">
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">
                            <div className="flex gap-2">
                                <SecondaryButton type="button" onClick={() => navigate("/enquiries/" + data?._id + "/edit")} className="rounded-full min-w-[94px] text-sm">
                                    View
                                </SecondaryButton>
                            </div>
                        </td>
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">{statusList[data.status]}</td>
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">{Moment(data?.createdAt).format("Y-M-D hh:mm:ss")}</td>
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">{data?.title}</td>
                    </tr>
                ))
            ) : (
