...
import FormStatusBadge from "@/components/common/DataBreachFormStatusBadge";
import { PrimaryButton } from "@/components/ui/buttons/primary-button";
import { SecondaryButton } from "@/components/ui/buttons/secondary-button";

const DataTable = ({ isLoading, tableColumn, dataSource, totalRecord, dataFilter, onChangePage, deleteRecord }) => {
                    ...
                    <tr key={index} className="bg-secondary/35 [&>td:last-child]:border-e-0">
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">
                            <div className="flex gap-2">
                                <SecondaryButton type="button" onClick={() => navigate("/data-breach/" + data?._id + "/edit")} className="rounded-full min-w-[94px] text-sm">
                                    View
                                </SecondaryButton>
                            </div>
                        </td>
                        ...
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">{data.entityName}</td>
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">{data.licenseNumber}</td>
                        <td className="py-3 px-6 border-t border-e border-secondary/60 ">{data.contact?.phone}</td>
